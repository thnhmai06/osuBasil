...
namespace OpenOsuTournament.Bancho.Application.UseCases.Mail;
...
{
    public async Task MarkChannelAsReadAsync(PlayerSession player, string channel,
        CancellationToken cancellationToken = default)
    {
        if (string.IsNullOrEmpty(channel)) return;

        var senderId = sessionRegistry.GetByName(channel)?.Id ??
                       (await users.FetchByNameAsync(channel, cancellationToken))?.Id;
