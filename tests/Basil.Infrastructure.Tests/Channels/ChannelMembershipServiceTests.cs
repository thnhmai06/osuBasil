...
namespace Basil.Infrastructure.Tests.Channels;
...
public class ChannelMembershipServiceTests
{
	...
	private ChannelMembershipService MakeService()
	{
		return new ChannelMembershipService(_gameRegistry, _ircRegistry, _channelRegistry,
			...
			_matchRegistry, _hub, Options.Create(new IrcOptions()), _userCache);
	}
	...
	public void Join_MatchChannel_DeniedForANonParticipantNonReferee()
	{
		...
		var match = new MatchSession(0, "Grand Finals", "", null, new User { Id = 9, Name = "host" },
			GameMode.Standard, Mods.NoMod, MatchWinCondition.Score, MatchTeamType.HeadToHead, false, 0) { DbId = 5 };
	...
	public void Join_MatchChannel_AllowedForAReferee_EvenBeforeEverPhysicallyJoining()
	{
		...
		var match = new MatchSession(0, "Grand Finals", "", null, new User { Id = 9, Name = "host" },
			GameMode.Standard, Mods.NoMod, MatchWinCondition.Score, MatchTeamType.HeadToHead, false, 0) { DbId = 5 };
		match.AddReferee(_userCache.Resolve(referee));
		_matchRegistry.All.Returns([match]);
		Register(referee);
	...
	public void Join_MatchChannel_AllowedForASeatedPlayer()
	{
		...
		var match = new MatchSession(0, "Grand Finals", "", null, new User { Id = 9, Name = "host" },
			GameMode.Standard, Mods.NoMod, MatchWinCondition.Score, MatchTeamType.HeadToHead, false, 0) { DbId = 5 };
		match.Slots[0].Player = _userCache.Resolve(seated);
		_matchRegistry.All.Returns([match]);
		Register(seated);
	...
	public void MemberPrefix_AuthorityIsStaffOutsideAMatchRoomAndTheRoomsRefereesInside()
	{
		...
		var room = new ChannelSession(0, "#mp_5", 0, 0, false, "#multiplayer", true);
		var refereeUser = _userCache.Resolve(referee);
		var match = new MatchSession(0, "Grand Finals", "", null, refereeUser, GameMode.Standard,
			Mods.NoMod, MatchWinCondition.Score, MatchTeamType.HeadToHead, false, 0) { DbId = 5 };
		match.AddReferee(refereeUser);
		_matchRegistry.All.Returns([match]);
	...
	public void BroadcastPrivmsg_InAMatchChannel_PublishesEveryLineIncludingTheSenderSOwn()
	{
		...
		var room = new ChannelSession(0, "#mp_5", 0, 0, false, "#multiplayer", true);
		var general = new ChannelSession(1, "#osu", 0, 0, true);
		var match = new MatchSession(0, "Grand Finals", "", null, _userCache.Resolve(speaker), GameMode.Standard,
			Mods.NoMod, MatchWinCondition.Score, MatchTeamType.HeadToHead, false, 0) { DbId = 5 };
		...
		_hub.Received(1).Publish(new StreamKey("match", 5, "chat"), Arg.Any<long>(), Arg.Any<ReadOnlyMemory<byte>>());
		_hub.DidNotReceive().Publish(
			Arg.Is<StreamKey>(k => k != new StreamKey("match", 5, "chat")), Arg.Any<long>(),
