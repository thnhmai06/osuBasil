...
using Basil.Application.Unresolved.Auth;
using Basil.Application.Unresolved.Multiplayer;
...
namespace Basil.Application.Unresolved.Scores;
...
///     (<see cref="MatchSession.CurrentRoundId" />) with the
	...
	ILogger<ScoreSubmissionService> logger)
{
		...
		CancellationToken cancellationToken = default)
	{
		...
		var beatmap = await beatmaps.SearchAsync(md5: beatmapMd5, cancellationToken: cancellationToken);
		if (beatmap is null)
		{
	...
	///     (<see cref="CachedPlayerStats.Rank" />), which holds the
