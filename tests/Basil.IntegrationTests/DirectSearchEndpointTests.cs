...
public class DirectSearchEndpointTests : IClassFixture<WebApplicationFactory<Program>>
{
	...
	public async Task Search_Authenticated_ReturnsFormattedResults()
	{
		...
		sessionRegistry.TryAddGameSession(new GameSession(60, "search-user", "tok", UserPrivileges.Unrestricted,
	...
	public async Task Search_MirrorConfiguredAndSucceeds_ReturnsMirrorResultsNotLocal()
	{
		...
		sessionRegistry.TryAddGameSession(new GameSession(63, "mirror-user", "tok4", UserPrivileges.Unrestricted,
			DateTimeOffset.UnixEpoch));
		_searchResult = [[MakeBeatmap(1, 100)]];
	...
	public async Task Search_MirrorConfiguredButFails_FallsBackToLocalResults()
	{
		...
		var sessionRegistry = mirrorFactory.Services.GetRequiredService<IUserSessionRegistry>();
		sessionRegistry.TryAddGameSession(new GameSession(64, "mirror-fail-user", "tok5", UserPrivileges.Unrestricted,
			DateTimeOffset.UnixEpoch));
		_searchResult = [[MakeBeatmap(1, 100)]];
	...
	{
		var sessionRegistry = _factory.Services.GetRequiredService<IUserSessionRegistry>();
		sessionRegistry.TryAddGameSession(new GameSession(61, "searchset-unknown", "tok2", UserPrivileges.Unrestricted,
			DateTimeOffset.UnixEpoch));
		_setInfo = null;
	...
	{
		var sessionRegistry = _factory.Services.GetRequiredService<IUserSessionRegistry>();
		sessionRegistry.TryAddGameSession(new GameSession(62, "searchset-known", "tok3", UserPrivileges.Unrestricted,
			DateTimeOffset.UnixEpoch));
