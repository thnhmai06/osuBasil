...
///     handlers that already hold <see cref="Features.Multiplayer.MatchSession.Lock" />, must never block on a slow or
...
///     <see cref="SseSubscriberRegistry" />, so a match's subscriptions
...
public interface IMatchLiveEvents
{
	...
	///     completed (see <see cref="SseSubscriberRegistry" />) so a
