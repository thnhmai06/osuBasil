...
```jsonc
{
  ...
  "live": null,           // null nếu trận đã đóng; object nếu trận đang mở trong bộ nhớ
  "events": [
    {
...
Ghi chú:
- `live.host` là **object** (`userId`/`userName`/`country`), không còn là `hostId` số nguyên ở top-level như trước.
- `live` = `null` ngay khi trận đóng (`!mp close`, hoặc mất referee cuối cùng).
- `events` là mảng các lifecycle event: Created(0), RoundStarted(1), RoundEnded(2), HostGranted(3),
  PlayerJoined(4), PlayerLeft(5), Kicked(6), Closed(7). `actorUserId` = `null` khi event từ system,
  != `null` khi từ lệnh `!mp` trong game.
- `rounds[].aborted` = `true` nếu round bị huỷ (`!mp abort` hoặc server startup recovery).
- `winMetric` là tên human-readable: `"score"`, `"accuracy"`, `"combo"`, `"scorev2"`.
- `winDiff`: solo = khoảng cách metric giữa #1 và #2; team = chênh lệch tổng metric 2 team; =0 nếu draw
  hoặc match có ≤1 người chơi.
- `scores[].submittedAt` = thời gian server nhận score (`ServerTime`) — khác `startedAt` của round.
- `rounds` luôn có mặt (mảng rỗng nếu chưa round nào chơi xong).
...
Ghi chú: `HostId` (số nguyên) trước đây được thay bằng object `Host` có cả `UserName`/`Country`.
Thêm `Mode`, `Mods`, `Freemods`, `Referees` so với phiên bản trước.
`Slots` nay kèm `UserName`/`Country` thay vì chỉ `UserId`.
...
`GET /matches` → mảng (`mode`/`winCondition`/`teamType`/`hostId` không còn ở cấp Match — các field này nằm trong từng `rounds` ở `GET /multi/{id}`):
