...
///     drift. The wording itself lives outside the code, in this slice's
///     <c>Locale/bot.en.json</c> fragment, so it can be edited without a rebuild; each member's
///     lookup key (via <see cref="LocaleCatalog.Get" />) is a dotted key mirroring the command it
///     belongs to, so a member and its wording cannot silently fall out of sync. Format strings use
///     <see cref="string.Format(string, object?[])" /> placeholders;
