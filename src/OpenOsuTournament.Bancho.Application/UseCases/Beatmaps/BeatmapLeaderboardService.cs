...
///     separate beatmap-gating from score-querying; OpenOsuTournament.Bancho's simpler no-pp, no-clan, no-osu!api
    ...
    IPlayerSessionRegistry sessionRegistry)
{
        ...
        PlayerSession player, BeatmapLeaderboardRequest request, CancellationToken cancellationToken = default)
    {
        ...
        if (!bmap.HasLeaderboard)
            return new BeatmapLeaderboardResult(BeatmapLeaderboardResultCode.NoLeaderboard, bmap.Status);
        ...
        if (!request.RequestingFromEditorSongSelect)
        {
            ...
            {
                var relations =
                    await relationships.FetchAllAsync(player.Id, RelationshipType.Friend, cancellationToken);
            ...
            {
                var best = await scores.FetchPersonalBestLeaderboardScoreAsync(bmap.Md5, mode, player.Id,
                    cancellationToken);
                if (best is not null)
                {
                    var rank = await scores.FetchPersonalBestLeaderboardRankAsync(bmap.Md5, mode, best.Score,
                        cancellationToken);
    ...
    {
        if (mode == player.Status.Mode) return;
