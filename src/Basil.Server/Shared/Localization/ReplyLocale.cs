...
/// <summary>
///     Loads the wording for <see cref="MpReplies" /> and <see cref="IrcReplies" /> from two
...
internal static class ReplyLocale
{
	...
	/// <summary>Resolves a <see cref="MpReplies" /> member's text by its <c>"Category.Member"</c> key.</summary>
	public static string BasilBot(string key)
	{
		return Resolve(BasilBotDocument.Value, "BasilBot.json", key);
	}

	/// <summary>Resolves an <see cref="IrcReplies" /> member's text by its <c>"Category.Member"</c> key.</summary>
