...
/// <summary>Handles the client's notification that the player does not have the match's beatmap.</summary>
/// <remarks>
///     Marks the player's slot as <see cref="Basil.Domain.Multiplayer.SlotStatus.NoMap" />, which
///     signals other clients that the player cannot play the selected map. The state update is
...
public sealed class MatchNoBeatmapHandler(MatchMembershipService matchMembership) : IBanchoPacketHandler
{
	...
	/// <summary>Processes the no-beatmap packet for the given player.</summary>
