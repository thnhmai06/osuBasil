...
/// <summary>Handles the client's notification that the player has the match's beatmap.</summary>
/// <remarks>
///     Marks the player's slot as <see cref="Basil.Domain.Multiplayer.SlotStatus.NotReady" />, which
///     signals other clients that the beatmap is present for this player and readiness is pending. The
...
public sealed class MatchHasBeatmapHandler(MatchMembershipService matchMembership) : IBanchoPacketHandler
{
	...
	/// <summary>Processes the has-beatmap packet for the given player.</summary>
