...
- The middleware buffers the response (swaps `Response.Body` for a `MemoryStream`, rewrites after `next()` completes) — skipped entirely (no buffering, passed straight through) for a route tagged `Basil.Web/Middleware/SseEndpointMarker` (an always-SSE route like `GET /matches/{id}/live`, which never produces a plain-JSON body) or when the incoming request's `Accept` header itself asks for `text/event-stream` on one of the several content-negotiated routes (`GET /matches/{id}` and its `hosts`/`refs`/`ban`/`slots`/`timer` sub-resources) — mirroring that route's own internal `WantsSse` check, so the buffering decision always matches what the handler is about to do. Buffering a live push stream until the handler completes would otherwise silently turn it into one that never delivers a single event until the connection closes.
- `Basil.Web/OpenApi/OpenApiExampleExtensions.WithExample` mirrors the same wrapping when attaching a fake-data example to a route's OpenAPI schema, gated on `OpenApiOperationTransformerContext.DocumentName == "basilapi"` — so the Scalar docs' examples show the real enveloped shape a caller actually receives, not the bare inner DTO.

**Consistent embedding, not bare ids.** Every user reference in a `basilapi` response embeds `{ id, name, country }` via one reused `UserBrief` record (`MatchLiveSnapshotBuilder.cs`) — never a bare `userId`. `UserBriefResolver` resolves it by trying the in-memory `IPlayerSessionRegistry` first (online players, no DB hit), falling back to the cached `IUserRepository` for offline ids; genuinely unresolvable ids in most contexts fall back to a placeholder (`"Unknown"`/`"xx"`) rather than being silently dropped from a list, reserving an actual `null` field for structural absence (no host, empty slot). Every beatmap reference embeds the real `Basil.Domain.Beatmaps.Beatmap` domain record directly — no separate flattened DTO — keyed by looking up the relevant stored `Md5` (never `Id`, since a beatmap's id can persist across a re-ingestion that changes its content, but its md5 can't) through the same cached `IMapRepository`; `null` once that md5 no longer resolves (the beatmap changed or was deleted since).
