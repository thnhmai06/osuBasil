...
public sealed class SqliteClientHashRepository(string connectionString) : IClientHashRepository
{
        ...
        string diskSerial, CancellationToken cancellationToken = default)
    {
        ...
        await connection.ExecuteAsync(
            ...
            VALUES (@UserId, @OsuPath, @Adapters, @UninstallId, @DiskSerial, datetime('now'), 1)
            ON CONFLICT (UserId, OsuPath, Adapters, UninstallId, DiskSerial)
            DO UPDATE SET LatestTime = datetime('now'), Occurrences = Occurrences + 1
            """,
            new
            {
