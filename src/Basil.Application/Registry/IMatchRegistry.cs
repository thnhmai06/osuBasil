...
namespace Basil.Application.Registry;
...
public interface IMatchRegistry
{
	...
	/// <param name="host">
	///     The userSession who created the room, or <see langword="null" /> when nobody holds gameplay
	///     host.
	/// </param>
	/// <param name="cancellationToken">A token that cancels the create operations.</param>
	/// <returns>The newly registered match.</returns>
	Task<MatchSession> CreateAsync(MatchCreationData data, User? host, CancellationToken cancellationToken = default);
