...
public class GetScoresEndpointTests : IClassFixture<WebApplicationFactory<Program>>
{
    ...
    private sealed class StubScoreRepository : IScoreRepository
    {
        public Task<IReadOnlyList<BeatmapLeaderboardScoreRow>> FetchBeatmapLeaderboardScoresAsync(
            ...
            Task.FromResult<IReadOnlyList<BeatmapLeaderboardScoreRow>>([]);
        ...
        public Task<PersonalBestLeaderboardScoreRow?> FetchPersonalBestLeaderboardScoreAsync(
            ...
            Task.FromResult<PersonalBestLeaderboardScoreRow?>(null);
    ...
    private static HttpRequestMessage MakeRequest(string queryString) =>
        new(HttpMethod.Get, $"/web/osu-osz2-getscores.php?{queryString}") { Headers = { Host = "osu.test.local" } };

    [Fact]
    public async Task PlayerNotOnline_ReturnsUnauthorized()
    {
        var client = _factory.CreateClient();
        var request = MakeRequest($"us=nobody&ha=x&s=0&vv=4&v=1&c={new string('1', 32)}&f=file.osu&m=0&i=-1&mods=0&h=x&a=0");
    ...
    public async Task WrongPassword_ReturnsUnauthorized()
    {
        ...
        var request = MakeRequest($"us=cmyui-wrongpw&ha=wrong-md5&s=0&vv=4&v=1&c={new string('2', 32)}&f=file.osu&m=0&i=-1&mods=0&h=x&a=0");
    ...
    public async Task UnknownBeatmap_ReturnsNotSubmitted()
    {
        ...
        _maps.Beatmap = null;
        var request = MakeRequest($"us=cmyui-notsubmitted&ha=correct-md5&s=0&vv=4&v=1&c={new string('3', 32)}&f=unknown.osu&m=0&i=-1&mods=0&h=x&a=0");
    ...
    public async Task RankedBeatmap_NoScores_ReturnsFoundWithHeaderLine()
    {
        ...
        var request = MakeRequest($"us=cmyui-found&ha=correct-md5&s=0&vv=4&v=1&c={md5}&f=file.osu&m=0&i=-1&mods=0&h=x&a=0");
