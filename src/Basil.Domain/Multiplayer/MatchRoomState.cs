...
public sealed class MatchRoomState(
	int id,
	string name,
	string password,
	string mapName,
	...
	string chatChannelName)
{
	...
	///     packet, or an <c>osump://</c> url). The host is not exempt: hosting only grants in-room
	///     settings control, not a standing invite, so a host who leaves a private room needs a
	///     referee's <c>!mp invite</c> like anyone else to get back in. Also hidden from
	///     <c>#lobby</c>. Distinct from <see cref="IsLocked" />, which blocks every non-member
	///     outright regardless of invite status. Defaults to <see langword="false" /> for all new
	///     rooms. Not persisted to the database: a room that closes while private loses this flag,
	///     along with its invite list.
	/// </summary>
	...
	///     canceled whenever the match is torn down so no announcement fires into a dead channel.
	/// </summary>
	...
	///     protocol actually uses as a match id). Set once, right after the room is created.
	/// </summary>
	public int DbId { get; set; }

	/// <summary>
	///     Gets or sets the id of the player who created this match, or null when the room was
	...
	/// <summary>Gets a value that indicates whether <paramref name="playerId" /> may issue <c>!mp</c> commands on this match.</summary>
	/// <remarks>
	...
	///     which ranks below referee authority for <c>!mp</c> purposes. Kept by player id, exactly
	...
	/// <summary>Gets the slot currently occupied by <paramref name="playerId" />, or null when the player is not in the match.</summary>
	...
	/// <summary>Gets the index of the slot currently occupied by <paramref name="playerId" />, or null when the player is not in the match.</summary>
	/// <param name="playerId">The id of the player to look up.</param>
