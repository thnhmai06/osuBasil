...
///     registered once via <c>ConfigureHttpJsonOptions</c> in <c>Program.cs</c> so it applies
///     everywhere a <see cref="Country" /> is embedded (never per-property, unlike
///     <see cref="TimeSpanSecondsJsonConverter" />, since every <see cref="Country" /> field wants
///     the same wire shape with no exceptions).
/// </summary>
