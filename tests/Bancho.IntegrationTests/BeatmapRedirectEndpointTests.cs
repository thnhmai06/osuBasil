...
{
    private static WebApplicationFactory<Program> Configure(WebApplicationFactory<Program> factory, string? downloadEndpoint = null) =>
        factory.WithWebHostBuilder(builder =>
        {
            builder.ConfigureAppConfiguration((_, config) =>
            {
                ...
                {
                    settings["Mirror:DownloadEndpoint"] = downloadEndpoint;
                }
    ...
    private static HttpRequestMessage MakeRequest(string path) =>
        new(HttpMethod.Get, path) { Headers = { Host = "osu.test.local" } };
    ...
    {
        var client = Configure(_factory).CreateClient(new WebApplicationFactoryClientOptions { AllowAutoRedirect = false });
