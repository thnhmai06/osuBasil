...
///     typo or one of osu!'s keys Basil has no data for (see <see cref="BeatmapsetQuery" />'s
...
public sealed partial record BeatmapsetQuery
{
	...
	private sealed partial class Builder
	{
		...
		public bool TryApply(string key, ComparisonOperator op, string rawValue)
		{
			switch (key)
			{
				case "stars" or "star": return TrySetDouble(rawValue, op, v => _star = v);
		...
		{
			return new BeatmapsetQuery(keywords,
				_bpm, _lengthSeconds, _cs, _ar, _od, _hp, _star, _keys, _circles, _sliders, _creator, _artist, _title,
