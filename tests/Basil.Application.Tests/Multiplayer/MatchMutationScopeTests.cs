...
public class MatchMutationScopeTests
{
	private static MatchSession NewMatch(List<long>? published = null)
	{
		var match = new MatchSession(
			0, "test match", "pw", null, new User { Id = 1, Name = "host" },
	...
	private static MatchSession NewMatchWhosePublishThrows()
	{
		var match = new MatchSession(
			...
			MatchTeamType.HeadToHead, false, 0)
		{
			MutationPublisher = new ThrowingPublisher()
		};
	...
	private static long VersionsAllocatedDuring(MatchSession match)
	{
		return match.CurrentStateVersion + 1;
	}
	...
	public async Task AMutationThatPublishesNothingAllocatesNoVersion()
	{
		...
		await using (var m = await match.BeginMutationAsync(CancellationToken.None))
		{
	...
	public async Task RepeatedPublishRequestsCoalesceToOneVersion()
	{
		...
		await using (scope = await match.BeginMutationAsync(CancellationToken.None))
		{
			scope.Session.Slots[0].Status = RoomSlotStatus.Ready;
			scope.PublishState();
			scope.PublishState();
	...
	{
		var match = NewMatch();
		await using var outer = await match.BeginMutationAsync(CancellationToken.None);
		...
		await Assert.ThrowsAsync<InvalidOperationException>(async () => await match.BeginMutationAsync(timeout.Token));
	}
	...
	public async Task AnExceptionInsideTheScopeStillPublishesWhatChanged()
	{
		...
		{
			await using var m = await match.BeginMutationAsync(CancellationToken.None);
			m.Session.Slots[0].Status = RoomSlotStatus.Ready;
			m.PublishState();
