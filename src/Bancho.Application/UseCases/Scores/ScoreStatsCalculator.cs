...
public static class ScoreStatsCalculator
{
    ...
    public static long RankedScoreDelta(ScoreSubmission score) =>
        score.PrevBest is null ? score.Score : score.Score - score.PrevBest.Score;
    ...
    public static IReadOnlyDictionary<Grade, int> GradeCountDeltas(ScoreSubmission score)
    {
        ...
        if (score.Grade == score.PrevBest.Grade)
        {
            return new Dictionary<Grade, int>();
        }
    ...
    public static CachedPlayerStats ApplyRankedScoreStats(ScoreSubmission score, CachedPlayerStats stats)
    {
        ...
        return stats with { XhCount = xh, XCount = x, ShCount = sh, SCount = s, ACount = a, Rscore = stats.Rscore + RankedScoreDelta(score) };
    }
    ...
    public static CachedPlayerStats ApplyScoreStats(ScoreSubmission score, CachedPlayerStats stats)
    {
        ...
        if (!score.Passed || score.Bmap is null || !score.Bmap.HasLeaderboardStrict)
        {
            return updated;
        }

        if (score.MaxCombo > updated.MaxCombo)
        {
            updated = updated with { MaxCombo = score.MaxCombo };
        }
