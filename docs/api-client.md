...
```
Server:Domain      (settings.toml, cạnh executable — cùng 1 file cho dev lẫn deployment)
```
...
| IRC gateway (TCP, không phải HTTP)                   | `irc.<domain>` (port 6667, không route qua ASP.NET)                      | `TcpIrcListener` (`BackgroundService`) |

`Program.cs:30` đọc `Server:Domain` và truyền nó vào `MapAll` — đó là nơi duy nhất domain được truyền qua;
không có config host theo từng route nào khác ở bất kỳ đâu.

**Giá trị config đó tự nó nằm ở đâu:** `src/Basil.Web/settings.toml`, section `[Server]`, key `Domain`
— cùng 1 file dùng cho cả `dotnet run` (dev cục bộ) lẫn executable đã publish, không có tách biệt theo môi
trường và không có lớp override qua biến môi trường. Sửa file, khởi động lại process. Xem
[`run-deployment.md`](run-deployment.md).
...
| `SendPublicMessage`       | `SendPublicMessageHandler`       | gửi một message tới channel — handler delegate tới `ChatDispatchService.SendPrivmsgAsync`, lớp này broadcast IRC-shaped qua `ChannelMembershipService.BroadcastPrivmsg` (tới cả osu! client lẫn IRC client), rồi route lệnh `!` tới `ICommandDispatcher`. Reply khác null được broadcast từ BasilBot tới cùng channel | Có BanchoBot với cùng cơ chế dispatch lệnh chat, nhưng bộ lệnh đầy đủ hơn nhiều (`stats`, `report`, mappool, scrim...) — xem [`bot-commands.md`](bot-commands.md) để biết chính xác lệnh nào Basil có/thiếu | ⚠️ Có nhưng có giới hạn |
| `SendPrivateMessage`      | `SendPrivateMessageHandler`      | gửi DM tới player khác — handler delegate tới `ChatDispatchService.SendPrivmsgAsync`, lớp này route bot-command shortcut (không cần prefix trong DM) qua `ICommandDispatcher`, hoặc check block/silence rồi deliver qua `IIrcConnection` (cả bancho bridge lẫn real IRC client) + lưu offline mail | Có, BanchoBot nhận lệnh qua DM giống hệt — cùng khác biệt bộ lệnh như trên                                                                                                                                  | ⚠️ Có nhưng có giới hạn |
