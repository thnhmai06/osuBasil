...
- **Routing is host-based, not path-based.** `Basil.Web/Routing/BanchoHostGroups.cs` mounts a different route group per subdomain (`c./ce./c4./c5./c6.` → bancho binary protocol, `osu.` → `/web/osu-*.php` HTTP endpoints, `b.` → legacy beatmap-thumbnail redirect to osu.ppy.sh's CDN, `a.` → local avatar files, `api.` → tournament match report (TRT) over GET/SSE, file downloads, and admin-key-gated management CRUD). See [`docs/api-client.md`](docs/api-client.md) for the `c./ce./c4./c5./c6.`/`osu.`/`b.`/`a.` packet and endpoint tables plus exactly where the hosting domain is configured, and [`docs/api-external.md`](docs/api-external.md) for the `api.` host's TRT/download/management-CRUD tables.
