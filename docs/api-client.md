...
```
ServerBehavior:Domain      (settings.toml, cạnh executable — cùng 1 file cho dev lẫn deployment)
```
...
`Program.cs:37` đọc `ServerBehavior:Domain` và truyền nó vào `MapAll` — đó là nơi duy nhất domain được truyền qua;
không có config host theo từng route nào khác ở bất kỳ đâu.

**Giá trị config đó tự nó nằm ở đâu:** `src/Basil.Web/settings.toml`, section `[ServerBehavior]`, key `Domain`
