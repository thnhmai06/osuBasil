...
internal static class AdminManagementRoutes
{
    ...
    private static void MapBeatmaps(RouteGroupBuilder admin)
    {
        ...
        admin.MapPost("/beatmaps/rescan", async (BeatmapIngestionService ingestion, CancellationToken cancellationToken) =>
        {
            var ingested = await ingestion.IngestAsync(cancellationToken);
            return Results.Json(new { ingested });
        });
    ...
    private static void MapUsers(RouteGroupBuilder admin)
    {
        ...
        {
            var passwordMd5 = System.Security.Cryptography.MD5.HashData(
                System.Text.Encoding.UTF8.GetBytes(body.Password));
            var pwBcrypt = passwordHasher.Hash(passwordMd5);
            var user = await users.CreateAsync(body.Name, body.Email, pwBcrypt, body.Country ?? "xx", cancellationToken);
    ...
    private static void MapMatches(RouteGroupBuilder admin)
    {
        admin.MapGet("/matches", async (IMatchPersistenceRepository matchPersistence,
            CancellationToken cancellationToken) => Results.Json(await matchPersistence.FetchAllMatchesAsync(cancellationToken)));
