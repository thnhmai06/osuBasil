...
///     is never a lock, so a burst of reconnects can never contend with it. A fresh subscriber reads
...
public sealed class SnapshotChannel<T> where T : class
{
	...
	/// <value>The latest published snapshot, or <see langword="null" /> before the first publish.</value>
	...
	/// <returns>The UTF-8 JSON bytes of the delta patch (or the full snapshot on first publish) to broadcast.</returns>
