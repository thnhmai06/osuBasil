    ...
    IOptions<DiscordOptions> discordOptions)
{
    ...
    public async Task<OsuLoginResult> ExecuteAsync(OsuLoginRequest request, CancellationToken cancellationToken = default)
    {
        ...
        {
            return InvalidRequestFailure(tokenOverride: "invalid-adapters");
        }

        if (!(runningUnderWine || adapters.Any(a => a.Length > 0)))
        {
            return InvalidRequestFailure(tokenOverride: "empty-adapters");
        }
        ...
        {
            return new OsuLoginResult("no", ServerPacketWriter.LoginReply((int)LoginFailureReason.AuthenticationFailed));
        }

        /* login credentials verified */

        await ingameLogins.CreateAsync(user.Id, request.Ip.ToString(), osuVersion.Date, StreamName(osuVersion.Stream), cancellationToken);

        await clientHashes.CreateAsync(
            user.Id, loginData.OsuPathMd5, loginData.AdaptersMd5, loginData.UninstallMd5, loginData.DiskSignatureMd5, cancellationToken);

        var diskSignatureForBanCheck = loginData.DiskSignatureMd5 == InactionableDiskSignatureMd5 ? null : loginData.DiskSignatureMd5;

        var hardwareMatches = await clientHashes.FetchAnyHardwareMatchesForUserAsync(
            user.Id, runningUnderWine, loginData.AdaptersMd5, loginData.UninstallMd5, diskSignatureForBanCheck, cancellationToken);
        ...
        if (user.Country == "xx")
        {
            await users.UpdateCountryAsync(user.Id, geoloc.CountryAcronym, cancellationToken);
        }

        var session = new PlayerSession(user.Id, user.Name, tokenGenerator.GenerateToken(), (Privileges)user.Priv, loginTime)
        {
        ...
        {
            if (!channel.CanRead((Privileges)user.Priv) || channel.Name == "#lobby")
            {
                continue;
            }
            ...
            {
                if (channel.CanRead((Privileges)other.Priv))
                {
        ...
        var userRelationships = await relationships.FetchAllAsync(user.Id, type: null, cancellationToken: cancellationToken);
        ...
        var userPresenceAndStats = Concat(PacketBuilders.BuildUserPresence(session), PacketBuilders.BuildUserStats(session));
        ...
        if (!session.Restricted)
        {
            ...
            foreach (var msg in unreadMail)
            {
                ...
                var msgTime = msg.Time.HasValue ? DateTimeOffset.FromUnixTimeSeconds(msg.Time.Value).UtcDateTime : DateTime.UtcNow;
                data.Add(ServerPacketWriter.SendMessage(msg.FromName, $"[{msgTime:ddd MMM d @ h:mmtt}] {msg.Msg}", msg.ToName, msg.FromId));
            }
    ...
    private static string StreamName(OsuStream stream) => stream switch
    {
    ...
    private OsuLoginResult IncorrectCredentials() =>
        new("incorrect-credentials", Concat(
