    ...
    IClock clock)
{
    ...
    private static void ApplyTeamType(MatchSession match, MatchTeamTypes newType)
    {
    ...
    private (bool Success, string? Reply) Set(MatchSession match, IReadOnlyList<string> args)
    {
        ...
        MatchWinConditions? winCondition = null;
        if (args.Count >= 2)
        {
    ...
    private static bool TryParseTeamType(string arg, out MatchTeamTypes teamType)
    {
        teamType = default;
        if (!int.TryParse(arg, out var value) || value is < 0 or > 3) return false;

        teamType = (MatchTeamTypes)value;
        return true;
    }

    private static bool TryParseWinCondition(string arg, out MatchWinConditions winCondition)
    {
        winCondition = default;
        if (!int.TryParse(arg, out var value) || value is < 0 or > 3) return false;

        winCondition = (MatchWinConditions)value;
        return true;
    }
        ...
        CancellationToken cancellationToken)
    {
        ...
        matchMembership.EnqueueState(match);
        return (true, $"Changed beatmap to {bmap.Artist} - {bmap.Title}");
    }
    ...
    private (bool Success, string? Reply) SetMods(MatchSession match, IReadOnlyList<string> args)
    {
        ...
        match.Mods = mods;
        matchMembership.EnqueueState(match);
        return (true, DescribeModChange(before, mods));
    }
    ...
    /// </summary>
    private static string DescribeModChange(Mods before, Mods after)
    {
