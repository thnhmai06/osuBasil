...
{
    public async Task<BeatmapResolutionResult> ResolveAsync(string md5, string filename, CancellationToken cancellationToken = default)
    {
        var bmap = await maps.FetchOneAsync(md5: md5, cancellationToken: cancellationToken);
        if (bmap is not null)
        {
            return new BeatmapResolutionResult(BeatmapResolutionResultCode.Found, bmap);
        }

        var existsByFilename = await maps.FetchOneAsync(filename: filename, cancellationToken: cancellationToken) is not null;
