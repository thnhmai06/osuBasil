...
///     (<see cref="Basil.Server.Features.Multiplayer.MatchSession.CurrentRoundId" />) with the
	...
	ILogger<ScoreSubmissionService> logger)
{
	...
	///     (<see cref="Basil.Server.Shared.Sessions.CachedPlayerStats.Rank" />), which holds the
	///     userSession's own user id instead (see
	///     <see cref="Basil.Server.Features.Auth.LoginService" />).
	/// </remarks>
