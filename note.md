...
**CHƯA COMMIT tại thời điểm ghi note này** — đang chờ full `Infrastructure.Tests` chạy nền làm baseline trước khi commit.

## Việc CÒN LẠI cho Phase 7

**Lưu ý theo đúng lời advisor**: scrim ENGINE đã xong nhưng CHƯA "hoạt động được" trong 1 trận thật — `IsScrimming`/`WinningPoints`/`MatchPoints` chỉ được set thủ công qua code/test, CHƯA có đường nào từ client thật bật nó lên (đó là việc của `!mp scrim <bestof>`/`!mp endscrim` — nằm trong slice `!mp` commands tiếp theo, KHÔNG gộp chung với engine này theo đúng lời dặn "giữ 2 việc là 2 commit riêng").

Sau commit engine scrim: `!mp`/`!pool` (25 lệnh admin qua chat command, KHÔNG phải packet — dùng `ICommandDispatcher`/`ICommand` đã có sẵn từ Phase 3/4; đây là nơi `!mp scrim`/`!mp endscrim`/`!mp addref`/`!mp ban` thực sự nối vào engine vừa xây), rồi HTML pages `/matches`/`/online`. 3 packet tourney đã xong (không phải hoãn nữa — xem commit 4/5 ở trên, phần "Việc CÒN LẠI" cũ đã lỗi thời, chỉ giữ referee/timer (`!mp start <seconds>` đếm ngược) là còn hoãn thật.
