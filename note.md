...
**Đã rà lại và chốt xong toàn bộ Phase 8/9/10 với user** (chi tiết đầy đủ trong `glowing-snacking-hickey.md`'s "Phase 7.5 — THU HẸP PHẠM VI TOÀN DỰ ÁN"):
- **Phase 8** — GIỮ port thật: `getbeatmapinfo`, `lastfm anticheat`, `mark-as-read`, `seasonal`, `bancho_connect`, `check-updates` (stub), `b.* redirect`. BỎ (stub trả response rỗng/hợp lệ): screenshot, favourites, rate, comments, `/users` registration, peppy joke, difficulty-rating redirect.
- **Phase 9 (API v1/v2)** — BỎ HOÀN TOÀN, hoãn vô thời hạn. Giải thích rõ cho user: đây là JSON API công khai cho công cụ BÊN NGOÀI (website/bot Discord/trang bracket giải đấu) — client osu! KHÔNG BAO GIỜ gọi tới đây (client chỉ gọi bancho protocol nhị phân + `/web/osu-*.php` + domain tĩnh osu./a./b.). User xác nhận: "vậy bỏ, tôi sẽ làm api sau".
- **Phase 10** — phần lớn tự động hoãn theo cascade (mọi lệnh moderator/admin/`!clan *` đều là chat command đã bỏ, `BotStatus` phụ thuộc BanchoBot đã bỏ). CHƯA hỏi riêng về BackgroundServices/Discord webhook/metrics — để ngỏ tới khi cần.

## Phase 8 — ĐÃ XONG toàn bộ endpoint theo phạm vi đã chốt

**Phát hiện khi đọc source cho `lastfm anticheat`**: cần cơ chế "restrict" (khoá tài khoản + ép logout + Discord webhook) mà bancho-net chưa có (Phase 10 hoãn). Hỏi lại user → **user quyết định: khi phát hiện cheat tool chỉ LƯU LẠI để đánh dấu, KHÔNG hành động gì** (không restrict/không kick). Điều này dùng được bảng `logs` đã có sẵn trong schema (`001_base.sql`, cột `from`/`to`/`action`/`msg`/`time`) — y hệt format Python's `Player.restrict()` ghi log, chỉ khác là không thực sự restrict.

**Bug phát hiện trong doc comment cũ của `ClientFlags.cs`**: nói sai rằng "LastFMFlags (2019) chưa từng được bancho.py đọc" — đọc lại `app/services/client_integrity.py`/`lastfm.php` xác nhận CÓ xử lý (`ClientIntegrityService.handle_lastfm_flags`). Đã sửa comment + thêm enum `LastFMFlags` (bit 14-22, tách biệt với `ClientFlags` cũ dùng cho score submission).

**Đã làm**:
- `ILogRepository`/`MySqlLogRepository` (mới) — insert đơn giản vào bảng `logs` có sẵn.
- `ClientIntegrityService` (`UseCases/Anticheat/`) — port `handle_lastfm_flags`, BỎ nhánh restrict/kick/random-ban-roll/notification/Discord-webhook, chỉ gọi `ILogRepository.CreateAsync(0, playerId, "lastfm_flag", reason)` khi phát hiện HQ_ASSEMBLY/HQ_FILE/REGISTRY_EDITS, luôn trả `StopSending` cho các nhánh nghi ngờ (khớp hành vi gốc — Python cũng luôn trả STOP_SENDING dù có restrict hay không).
- `BeatmapInfoService` (`UseCases/Beatmaps/`) — port `fetch_beatmap_info` cho `getbeatmapinfo.php`. Tận dụng lại `IScoreRepository.FetchPersonalBestLeaderboardScoreAsync` đã có sẵn từ Phase 5/6 (không cần thêm repository method mới) để lấy grade theo vanilla mode hiện tại của người chơi. Thêm bảng mapping `RankedStatus → osu!api status` (Python's `bancho_to_osuapi_status` dict) — port nguyên văn kể cả việc KHÔNG map `UpdateAvailable`/`NotSubmitted` (sẽ throw `KeyNotFoundException` giống Python's `KeyError` nếu gặp, đúng tinh thần "port trung thực").
- `MailReadService` (`UseCases/Mail/`) — port `mark_channel_as_read` cho `markasread.php`. Phát hiện: tham số "channel" trong Python thực ra là TÊN NGƯỜI GỬI mail, không phải kênh chat (tàn dư đặt tên từ code cũ) — resolve online-first (`IPlayerSessionRegistry`) rồi fallback DB (`IUserRepository`), tận dụng lại `IMailRepository.MarkConversationAsReadAsync` đã có sẵn.
- Route mới trong `BanchoHostGroups.cs` (`MapOsuWebGroup`): `getbeatmapinfo.php` (POST, JSON body `{Filenames, Ids}` — nhánh `Ids` port y hệt Python là code chết, chỉ đọc không xử lý), `lastfm.php`, `markasread.php`, `getseasonal.php` (trả `[]` — không có tính năng background sự kiện nên không đọc config nào), `bancho_connect.php`/`check-updates.php` (trả response rỗng, không cần auth giống Python).
- Route STUB (bỏ, không xử lý logic): `osu-screenshot.php` (400 + thông báo "not available", theo đúng style đã có ở `/d/{id}` và `/web/maps/{file}`), `osu-getfavourites.php`/`osu-addfavourite.php` (rỗng), `osu-rate.php` (tái dùng response "not ranked" — 1 trong các response THẬT của Python, không phải bịa), `osu-comment.php` (rỗng), `POST /users` (tái dùng NGUYÊN VĂN JSON `INGAME_REGISTRATION_DISALLOWED_ERROR` — 1 response THẬT có sẵn trong Python khi tắt đăng ký in-game, không phải tự bịa), `POST /difficulty-rating` (307 redirect sang osu.ppy.sh — port y hệt, Python cũng luôn redirect vô điều kiện, không phải rút gọn).
- `b.*` redirect (`MapBeatmapAssetGroup`) — route catch-all `{**path}` redirect 301 sang `https://b.ppy.sh{path}`, thay route placeholder cũ. Ban đầu thử `MapFallback` nhưng không hoạt động đúng với `RequireHost` (trả 404) — đổi sang `MapGet("/{**path}", ...)` thì đúng.

**Sửa test cũ bị lỗi thời**: `HostRoutingTests.BeatmapAssetSubdomain_RoutesToMapGroup` từng assert route gốc `b.{domain}` trả text "map" (placeholder cũ) — sửa lại assert redirect 301 sang `b.ppy.sh/` (hành vi thật mới).
