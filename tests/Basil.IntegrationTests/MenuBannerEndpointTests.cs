...
public class MenuBannerEndpointTests : IClassFixture<WebApplicationFactory<Bootstrap>>, IDisposable
{
	...
	public async Task PostBanner_ExternalUrl_CreatesEntry()
	{
		...
		var response = await _factory.CreateClient()
			.SendAsync(MakeCreateRequest("https://example.test/banner.png", "https://example.test/event", begins,
				expires), TestContext.Current.CancellationToken);

		Assert.Equal(HttpStatusCode.Created, response.StatusCode);
		var body =
			await response.Content.ReadFromJsonAsync<Envelope<JsonElement>>(TestContext.Current.CancellationToken);
	...
	public async Task PatchBanner_UpdatesUrl()
	{
		...
		var createResponse = await client.SendAsync(MakeCreateRequest("https://example.test/b.png",
			...
			TestContext.Current.CancellationToken);
		var created =
			await createResponse.Content.ReadFromJsonAsync<Envelope<JsonElement>>(TestContext.Current
				.CancellationToken);
		...
		var patched =
			await patchResponse.Content.ReadFromJsonAsync<Envelope<JsonElement>>(TestContext.Current.CancellationToken);
	...
	public async Task GetMenuContent_ReturnsBannersWithCorrectIsCurrent()
	{
		...
		await client.SendAsync(MakeCreateRequest("https://example.test/current.png", "https://example.test",
			DateTime.UtcNow.AddDays(-1), DateTime.UtcNow.AddDays(1)), TestContext.Current.CancellationToken);
		await client.SendAsync(MakeCreateRequest("https://example.test/future.png", "https://example.test",
			DateTime.UtcNow.AddDays(10), DateTime.UtcNow.AddDays(20)), TestContext.Current.CancellationToken);

		var response = await client.SendAsync(MakeRequest(HttpMethod.Get, "/menu-content.json",
			"assets.test.local", null), TestContext.Current.CancellationToken);
	...
	public async Task PostBanner_NullBeginsAndExpires_IsAlwaysCurrentWithNullBoundsInManifest()
	{
		...
		var manifestResponse =
			await client.SendAsync(MakeRequest(HttpMethod.Get, "/menu-content.json", "assets.test.local", null),
				TestContext.Current.CancellationToken);
		var manifest =
			await manifestResponse.Content.ReadFromJsonAsync<JsonElement>(TestContext.Current.CancellationToken);
	...
	public async Task PostBanner_OnlyExpiresSet_BeginsHasNoLowerBound()
	{
		...
		var response = await client.SendAsync(MakeCreateRequest("https://example.test/expiring.png",
			"https://example.test", null, DateTime.UtcNow.AddDays(1)), TestContext.Current.CancellationToken);
		var created =
			await response.Content.ReadFromJsonAsync<Envelope<JsonElement>>(TestContext.Current.CancellationToken);
