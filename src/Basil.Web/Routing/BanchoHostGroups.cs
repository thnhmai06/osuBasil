...
/// <summary>
///     Gives this static class a concrete <c>ILogger&lt;T&gt;</c> category to log under, since
///     <see cref="BanchoHostGroups" /> itself can't be used as a type argument.
/// </summary>
internal sealed class BanchoHostGroupsLog;

/// <summary>
///     Registers each host group for both the fallback `ppy.sh` domain and the configured one: the
///     `c.`/`ce.`/`c4.`/`c5.`/`c6.` hosts carry the bancho realtime protocol, `osu.{domain}` the
///     osu! web endpoints, `b.{domain}` beatmap assets, `a.{domain}` avatars, and `api.{domain}`
///     the developer-facing API. Routes are picked by hostname, not by path prefix.
/// </summary>
public static class BanchoHostGroups
{
	...
	/// <summary>
	///     Mounts each of the five host groups (bancho, `osu.` web, `b.` beatmap assets, `a.` avatars,
	///     `api.`) at "/" restricted to their respective hostnames, for both the configured domain and
	///     the fallback `ppy.sh`.
	/// </summary>
	/// <param name="app">The web application to register the host-restricted route groups on.</param>
	/// <param name="configuredDomain">
	///     The server's configured `Server:Domain` (falling back to `localhost`), mounted alongside
	///     `ppy.sh`.
	/// </param>
	...
	/// <summary>
	///     Zips a locally-ingested beatmapset's whole storage folder (audio/images/video/.osu, as
	///     extracted) into a fresh .osz on the fly. Shared by /d/{setId} and the api. host's
	///     /beatmapsets/{id}/download (see BeatmapsetRoutes.cs). With <paramref name="noVideo" />, any
	///     video file is dropped and " [no video]" is appended to the returned filename. Returns null
	///     when the set has no local folder, or the folder is empty.
	/// </summary>
	internal static async Task<(byte[] Bytes, string FileName)?> BuildOszArchiveAsync(
	...
	/// <summary>
	///     Writes an embedded resource to <paramref name="destinationPath" />. The bytes land in a fresh
	///     temp file first and get renamed into place, so concurrent first-use requests that materialize
	///     the same fallback image all end up with a complete file instead of fighting over
	///     <c>File.Create</c> on the shared destination. Silently does nothing if the resource is
	///     missing.
	/// </summary>
	/// <param name="resourceName">The fully-qualified name of the embedded resource to extract.</param>
	/// <param name="destinationPath">The final file path to write the resource to.</param>
		...
		IAudioExtractor extractor, CancellationToken cancellationToken)
	{
		...
		// A -1/null PreviewTime means "no custom preview point set". Real osu! would cut that at 40%
		// into the track; this server just starts from the top of the file, since it's a private
		// offline server rather than osu!'s curated preview-point flow.
	...
	// CRUD, a trivial /health probe, and (below) the generated OpenAPI/Scalar documentation site. The
	// content is the same whether this server is self-hosted or run locally. The GitHub Pages copy is
	// a separate, fully static build of the same 3 pages (see docs-site/ and the CI workflow), with
	// Try-it-out disabled since there's no live backend behind it.
	/// <summary>
	///     Registers the `api.` host's routes: the TRT snapshot and per-resource SSE live channels, file
	///     downloads, admin-key-gated management CRUD, a trivial `/health` probe, the abbreviation
	///     redirects, and the generated OpenAPI/Scalar documentation site.
	/// </summary>
	...
	private static void MapApiGroup(this RouteGroupBuilder group)
	{
		...
		group.MapGet("/health", () => Results.Json(new HealthStatus("ok")))
			...
			.WithDescription("`{ status: \"ok\" }` with no dependency checks. The database is an embedded " +
			                 "SQLite file and is always available once the process is up. Public, no authentication.")
			.WithTags("Health")
	...
	// NOTE: placeholder handlers still cover the other host groups. Real endpoints are registered in
	// MapOsuWebGroup, MapBeatmapAssetGroup, and MapApiGroup respectively.
	extension(RouteGroupBuilder group)
	{
		/// <summary>
		///     Registers the bancho binary-protocol host group: a `GET /` liveness stub plus the `POST /`
		...
		private void MapBanchoGroup()
		{
			group.MapGet("/", () => "cho")
				...
				.WithDescription("Returns the literal string \"cho\". The real osu! client never sends this; it " +
				                 "only exists as a liveness probe for this host.")
			...
			group.MapPost("/", async (HttpContext context, CancellationToken cancellationToken) =>
				{
					...
					if (string.IsNullOrEmpty(token))
					{
						...
						if (!headers.ContainsKey("CF-Connecting-IP") && !headers.ContainsKey("X-Forwarded-For"))
						{
							...
							// need synthesising together, not just one.
				...
				.WithDescription("Without an `osu-token` request header, the body is the client's login block " +
				                 ...
				                 "client since its last poll (a long-poll protocol: the client calls this endpoint " +
				                 "repeatedly). An unrecognized `osu-token` (e.g. after a server restart) gets a " +
		...
		private void MapOsuWebGroup()
		{
			...
			// Reduced to a status-only reply: this server doesn't support browsing a beatmap's
			// leaderboard (out of scope), but it still reports the map's real BeatmapStatus
			// (matching osu-search.php/osu-search-set.php) instead of the "-1|false" (NotSubmitted)
			// stub used when there's no leaderboard, so Song Select's map icon doesn't disagree with
			...
			// Queries the local beatmaps table instead of proxying a mirror API, so it runs fully
			// offline.
			...
			group.MapGet("/d/{mapSetId}",
					async (string mapSetId, HttpContext context, CancellationToken cancellationToken) =>
					{
						...
						if (int.TryParse(rawSetId, out var setId))
						{
							...
							if (osz is not null)
								return Results.File(osz.Value.Bytes, "application/x-osu-beatmap-archive",
									osz.Value.FileName);
						}
				...
				.WithDescription("`{mapSetId}` may carry a trailing `n` to request a no-video archive. If the set " +
				                 ...
				                 "is configured. Prefer `GET /beatmapsets/{id}/download` on the Basil API for external " +
				                 "tooling; this route exists specifically for the in-game client.")
				.WithTags("Beatmaps");

			// Serves the locally ingested .osu file directly instead of redirecting to an external
			// host (this server runs fully offline).
			group.MapGet("/web/beatmaps/{mapFilename}", async (string mapFilename, HttpContext context,
					CancellationToken cancellationToken) =>
				{
					...
					return File.Exists(osuPath)
						? Results.File(osuPath, "application/x-osu-beatmap")
						: Results.NotFound();
				})
				.WithGroupName("osuweb")
			...
			// The "score" field name is reused by the client for both the base64 score-data string
			...
			group.MapPost("/web/osu-submit-modular-selector.php",
				...
				.WithDescription("Multipart form submission the client sends after a play completes. The `score` " +
				                 ...
				                 "post-score screen data (rank, PP-less score charts, etc., in osu!'s wire format). It never " +
			...
			// Reduced to a stub: per-map grade lookup is out of scope (this server doesn't support
			// browsing beatmap leaderboards).
			group.MapPost("/web/osu-getbeatmapinfo.php", async (
				...
				.WithDescription("Authenticates the caller, then always returns an empty body. Per-map grade " +
				                 "history (used by the real client's Song Select grade icons) is out of scope, since this " +
				                 "server doesn't track a leaderboard to grade against.")
				.WithTags("Stubs");

			// Detected cheat-tool flags are only logged (ClientIntegrityService), per an explicit user
			// decision: there's no restrict/kick machinery, just a log for manual review.
			...
			// No offline-mail persistence exists here (chat is online-only), so this is just an
			// auth-gated no-op to keep the client happy.
			...
			// Lists StorageOptions.SeasonalsPath: this server has no config-file list, just a folder
			// an admin drops images into (served back by the /seasonal/{file} route below).
			...
			// Always an empty stub response (this server does not manage or distribute client updates).
			...
			group.MapGet("/web/osu-rate.php", () => Results.Text("not ranked", "text/html", Encoding.UTF8))
				...
				.WithDescription("Always returns the literal `not ranked` response osu! itself uses for beatmaps " +
			...
			// fired while the user is still filling the form (one per field, as they tab through). Only
			...
			group.MapPost("/users", async (HttpContext context, IUserRepository users,
					...
					ILogger<BanchoHostGroupsLog> logger, CancellationToken cancellationToken) =>
				{
					...
					var user = await users.CreateAsync(username, pwBcrypt, Country.Xx,
						cancellationToken: cancellationToken);
					if (user is null)
						return Results.Json(
							new { form_error = new { user = new { username = new[] { "Username already taken." } } } },
			...
			// The real client expects this to open a difficulty-rating webpage in the user's system
			// browser (the response isn't parsed by the client itself); this server has no such
			// webpage, so it computes the star rating locally with IOsuCalculator instead and caches
			// the NoMod result onto Beatmaps.Sr. `b` (beatmap id) and `mods` (bitmask) are read from
			// the query string since the real client sends neither a body nor documented form fields
			// for this endpoint.
	...
	extension(RouteGroupBuilder group)
	{
		...
		///     or trimmed on demand and cached on disk after the first request.
		/// </summary>
		private void MapBeatmapAssetGroup()
		{
			...
			group.MapGet("/thumb/{setId:int}l.jpg", HandleThumbnailLarge)
				...
				.WithSummary("Beatmapset list-icon thumbnail (160x120, resized and cached).")
			...
			group.MapGet("/preview/{setId:int}.mp3", HandleAudioPreview)
				.WithGroupName("beatmapassets")
				.WithSummary("Beatmapset audio preview (10s mp3 clip, cut and cached).")
