...
public class BanchoPacketDispatcherTests
{
    private sealed class FakeHandler(ClientPackets packetId, bool allowedWhenRestricted, Action<PlayerSession, BanchoPacketReader> onHandle) : IBanchoPacketHandler
    {
        ...
        public void Handle(PlayerSession player, BanchoPacketReader reader) => onHandle(player, reader);
    }
    ...
    [Fact]
    public void Dispatch_KnownPacket_InvokesHandlerWithReaderPositionedAfterHeader()
    {
        var called = false;
        ...
        dispatcher.Dispatch(session, PacketBytes(ClientPackets.Ping, []));

        Assert.True(called);
    }

    [Fact]
    public void Dispatch_UnknownPacket_SkippedWithoutError()
    {
        var dispatcher = new BanchoPacketDispatcher([]);
        var session = new PlayerSession(1, "cmyui", "token", Privileges.Unrestricted, 0.0);

        dispatcher.Dispatch(session, PacketBytes(ClientPackets.CantSpectate, [1, 2, 3, 4]));
        // no exception -> success
    }

    [Fact]
    public void Dispatch_MultiplePacketsInOneRequest_InvokesEachHandler()
    {
    ...
    [Fact]
    public void Dispatch_RestrictedPlayer_OnlyInvokesRestrictedAllowedHandlers()
    {
        ...
        dispatcher.Dispatch(restrictedSession, body);
