...
Last updated: 2026-09-07T18:20:00Z (local 2026-09-08 01:20 UTC+7)
...
| --- | --- | --- | --- |
| 0 Foundation | Implementing | nothing | Sonnet worker (Tasks 0.2 finish + 0.3) |
...
| 2026-09-08 01:20 UTC+7 | finish Task 0.2, then Task 0.3 | running |
...
## Known hazards handed to the current worker

1. The SQL migrations are an `EmbeddedResource` resolved by assembly and filtered by resource name.
   After the merge the names change, and **an empty migration set is indistinguishable from success**
   — the runner reports nothing pending and the app starts against an unmigrated database. Proven by
   diffing a freshly migrated `sqlite_master` against `baseline/schema.txt`.
2. Deleting the two `AssemblyMarker` types breaks the architecture tests that take an assembly by
   `typeof`. Only those are removed in Task 0.3; the string-based Domain/Protocol purity tests still
   compile and still assert something true. The expected total becomes `1622 − removed`, restored by
   Task 0.4.
3. `Data/Localization/*.json` must still reach the output directory, or `ReplyLocale` fails at the
   startup touch rather than at build time.
...
| 2026-09-08 01:20 UTC+7 | 2026-09-08 06:17 UTC+7 | pending |
...
Review the repository state the current worker leaves behind — build, test count arithmetic, and
the three hazard checks above — then delegate Task 0.4 (rewrite the architecture tests, including
the cross-slice adjacency allowlist derived from the real compile errors).
