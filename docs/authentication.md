...
- **Why "one session per account, except tourney spectators"?** Tourney spectator clients are meant to run several at once, alongside the player's real client, to watch a match from multiple angles. Every other client type back closing the older session on relogin, so nothing accumulates.
