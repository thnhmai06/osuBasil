...
/// <summary>
///     Ported from app/packets.py's PacketReader. bancho.py's own test suite (packets_test.py)
///     only covers the write side, so these expected values were generated by feeding synthetic byte
///     sequences directly into the real Python PacketReader (see
...
public class PacketReaderTests
{
	...
	public void ReadReplayFrame()
	{
		...
		Assert.Equal(new ReplayFrame(Keys.Left1 | Keys.Left2, TaikoByte.None, 12.5f, -3.5f, 100), frame);
	}
	...
	public void ReadReplayFrameBundle_V1_TwoFrames_HasNoComboOrBonusPortion()
	{
		...
		Assert.Equal(new ReplayFrame(Keys.Left1 | Keys.Left2, TaikoByte.None, 100.5f, 200.25f, 1000),
			bundle.Frames[0]);
		Assert.Equal(new ReplayFrame(Keys.None, TaikoByte.None, 150.0f, 250.0f, 1016), bundle.Frames[1]);
		Assert.Equal(new ScoreFrame(1016, 0, 10, 2, 1, 0, 0, 0, 123456, 50, 12, true, 100, 0, false),
			bundle.ScoreFrame);
	}
