...
Basil never talks to osu!'s servers or a beatmap mirror. Every beatmapset lives on disk next to the executable, and the database is a reflection of that folder, not the other way around.

## Why local, offline storage?

A tournament server needs to work on a LAN with no internet access at all — venue Wi-Fi is not something to depend on mid-tournament. Keeping beatmaps as local files an organizer drops into a folder (or uploads through the admin API) means the server never has a hard dependency on anything outside itself for the maps it serves.
